...
- **Configurable single-merchant, not multi-tenant** — one deployment = one merchant; multiple merchants = multiple deployments. *Why not multi-tenant:* keeps the simplicity (no tenant_id/routing/token engine) while enabling standalone + open source.
- **Operator settings in the DB (Filament), secrets in `.env`** — `.env` carries infrastructure, secrets (incl. SMTP) and the deployment's visual identity (theme overlay, brand name, logo); operator-editable legal & operational content (offered languages, legal pages, withdrawal scope) lives in the DB and is edited in the Filament panel. *Why:* a non-technical operator configures the live behaviour without touching files, while visual identity travels with the deploy tier. See `design/configuration.md`.
...
- **Neutral-first theming** — neutral default design, optional brand theme via `APP_THEME` (the `--wf-*` overlay is code, selected per deployment); brand name + logo via `.env`. *Why:* branding is configuration, not a code fork.
- **Open-source split (AGPL-3.0)** — code incl. generic infra templates public (placeholders only); real URLs/secrets/brand assets stay out of the repo — secrets + branding in the server `.env` (brand overlays in the private repo), legal/operational content in the deployment's DB. *Why:* others can use it, no secrets in the repo; AGPL guards against closed-source reuse.
...
- **Operator backend defaults to English; consumer form is German** — the operator panel locale is set independently via `BACKEND_LOCALE` (default `en`), decoupled from the consumer-facing form which stays on `APP_LOCALE=de`. *Why:* the two audiences are independent — the standalone/OSS operator UI defaults to neutral English, while the end-user withdrawal form is German for the German launch; one shared locale would force a wrong default on one side. (slice-007)
